...
export const ChapterEditorHeader: React.FC<ChapterEditorHeaderProps> = ({
  ...
  return (
    <>
      {/* Main header - stack vertically on mobile, with auto-hide on scroll */}
      <div className={`flex flex-col gap-2 xs:gap-3 p-2 xs:p-3 sm:p-4 md:p-6 border-b border-zinc-700 bg-zinc-900/50 sticky top-0 z-30 transition-transform duration-300 ease-in-out ${isMobile && !isVisible ? '-translate-y-full' : 'translate-y-0'
        }`}>
        ...
        <div className="flex items-center gap-1.5 xs:gap-2 overflow-x-auto scrollbar-hide -mx-2 px-2 xs:-mx-3 xs:px-3 sm:mx-0 sm:px-0 pb-0.5">
          <button
            ...
            className={`px-2 xs:px-3 sm:px-4 py-1.5 xs:py-2 rounded-lg text-xs xs:text-sm font-semibold transition-all duration-200 whitespace-nowrap flex-shrink-0 flex items-center gap-1 ${showHistory || activeTab === 'history'
          ...
          <button
            onClick={onToggleTTS}
            className={`px-2 xs:px-3 sm:px-4 py-1.5 xs:py-2 rounded-lg text-xs xs:text-sm font-semibold transition-all duration-200 whitespace-nowrap flex-shrink-0 flex items-center gap-1 ${showTTS
              ? 'bg-amber-600 hover:bg-amber-500 text-white'
              : 'bg-zinc-800 hover:bg-zinc-700 text-zinc-300'
              }`}
